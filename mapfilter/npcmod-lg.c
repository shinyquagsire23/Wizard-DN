...
//Hook in 080598CC -- same as FR
//Null out bytes 08059A28-08059A2F -- same as FR
//Null out bytes 08059A12-08059A13 -- same as FR

void filter(unsigned int palette, unsigned int dest, unsigned int mode)
{
	int type = dest;
	if(palette == 0x08059A0F) // Palette offset same as FR
...
void doBW(unsigned char type, unsigned int thingy, unsigned int thingy2, unsigned int offset, unsigned int length)
{
	if(bwFilter[0] & 0x1 == 0x1)
	{
		if(bwFilter[0] == 0x3)
		{
			int (*func)(unsigned int, unsigned int) = (int (*)(void))0x08111F10+1; // Different
			func(thingy,thingy2);
		}
		int (*func2)(unsigned int, unsigned int) = (int (*)(void))0x08071544+1; // Same
		func2(offset,length);
	}
	else if(bwFilter[0] == 0x2)
	{
		int (*func2)(unsigned int, unsigned int) = (int (*)(void))0x080715F4+1; // Same
		func2(offset,length);
	}
}
